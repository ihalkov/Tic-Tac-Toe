...
function announce(announcer, endResult, currentPlayer) {
    // announcer is in a variable because is used more than one time
    // some colors can be added with relevant span element
...
function checkForWin(board, announcer, currentPlayer) {
    ...
    if (isWin) {
        ...
        // console.log(endStates[key]);
        // announce(endStates[key]);
        announce(announcer, endStates[key], currentPlayer)
        return false;
    } else if (!board.includes('')) {
        announce(announcer, endStates['tie'], currentPlayer)
        return false;
    } else {
        return true;
    }
